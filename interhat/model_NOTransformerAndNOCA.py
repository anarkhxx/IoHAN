...
class InterprecsysBase:
    ...
    def build_graph(self):
        ...
        #目标是将features_split_1-3这3个从(?,?,12)  ==>(?,12)
        features_split_1=tf.squeeze(
            tf.layers.dense(
                features_split_1,
                units=1,
                activation=None,
        ...
        test_output1, test_laststate1 = tf.nn.dynamic_rnn(cell=cell1, inputs=tf.stack([features_split_1], axis=1),
                                                          dtype=tf.float32)
        agg_feat_1 = test_laststate1
        # 2
        # cell2 = tf.contrib.rnn.BasicLSTMCell(num_units=50, state_is_tuple=False)
        test_output2, test_laststate2 = tf.nn.dynamic_rnn(cell=cell1, inputs=tf.stack([features_split_2], axis=1),
                                                          dtype=tf.float32)
        agg_feat_2 = test_laststate2
        # 3
        # cell3 = tf.contrib.rnn.BasicLSTMCell(num_units=50, state_is_tuple=False)
        test_output3, test_laststate3 = tf.nn.dynamic_rnn(cell=cell1, inputs=tf.stack([features_split_3], axis=1),
                                                          dtype=tf.float32)
        ...
        with tf.name_scope("Agg_visit_order") as scope:
            ...
            #无意义操作1=2=3=k,对比实验懒得改main.py
            self.attn_1 = self.attn_k
            self.attn_2 = self.attn_k
            self.attn_3 = self.attn_k
